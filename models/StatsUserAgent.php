...
class Table_StatsUserAgent extends Omeka_Db_Table
{
     ...
     *
     * We use $db->prefix directly rather than $db->StatsUserAgent or
     * getTableName() because:
     * - $db->MagicName only works for core Omeka models (Item, User, etc.)
     * - getTableName() is not available in all Omeka Classic versions
     * - $db->prefix is always set and equals the configured table prefix
     *   (default "omeka_"), so the result is always correct.
     *
     * @return string  e.g. "omeka_stats_user_agents"
     */
    private function _tableName()
    {
        return $this->getDb()->prefix . 'stats_user_agents';
    }
