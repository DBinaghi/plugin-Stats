...
class Table_Hit extends Omeka_Db_Table
{
	...
	public function getMostFrequents($field, $userStatus = null, $limit = null, $page = null)
	{
		...
		// $params['sort_field'] = array(
			// 'hits' => 'DESC',
			// This order is needed in order to manage ex-aequos, but also slows down a lot the query
			// 'added' => 'ASC',
		// );
		$params['sort_field']['hits'] = 'DESC';
		if (get_option('stats_sort_exaequo')) {
			// This order is needed in order to manage ex-aequos, but also slows down a lot the query
			$params['sort_field']['added'] = 'ASC';
		}
	...
	*
	...
	public function getEndDate($isLast = false)
	{
		$alias = $this->getTableAlias();
		$select = $this->getSelect();
		$select
			->reset(Zend_Db_Select::COLUMNS)
			->columns(array('added'))
			->order(array("$alias.added " . ($isLast ? "DESC" : "ASC")))
			->limit(1);
